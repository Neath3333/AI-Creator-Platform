  ...
  handler: async (ctx, args) => {
    ...
    const rankedSuggestions = suggestions
      ...
      .sort((a, b) => {
        ...
        const aRecent = a.lastPostAt !== null && a.lastPostAt !== undefined && a.lastPostAt > Date.now() - 7 * 24 * 60 * 60 * 1000;
        const bRecent = b.lastPostAt !== null && b.lastPostAt !== undefined && b.lastPostAt > Date.now() - 7 * 24 * 60 * 60 * 1000;
