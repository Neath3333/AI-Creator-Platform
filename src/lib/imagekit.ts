...
export const buildTransformationUrl = (src, transformations = []) => {
  ...
  const transformParams = transformations
    .map((transform) => {
      ...
      // Handle single effect - ImageKit format: e-{effect}
      if (transform.effect) params.push(`e-${transform.effect}`);

      // Handle multiple effects - combine with commas
      if (transform.effects && Array.isArray(transform.effects)) {
        transform.effects.forEach(effect => {
