...
export default function ImageUploadModal({
  ...
  const applyTransformations = async () => {
    ...
    try {
      ...
      // Background removal and drop shadow - combine into single transformation
      const effects = [];
      if (watchedValues.backgroundRemoved) {
        effects.push("bgremove");
      }
      ...
      if (effects.length > 0) {
        transformationChain.push({
          effects: effects  // Pass array of effects to combine
        });
      }
      ...
      // AI transformations (like background removal) take longer to process
      // Add cache busting to force fresh image load
      const cacheBustedUrl = `${transformedUrl}?t=${Date.now()}`;

      // Longer delay for AI transformations to process on ImageKit's servers
      const hasAITransform = transformationChain.some(
        t => t.effects?.includes('bgremove') || t.effects?.includes('dropshadow')
      );
      const delay = hasAITransform ? 3000 : 1500;

      await new Promise((resolve) => setTimeout(resolve, delay));

      setImageLoadError(false);
      setTransformedImage(cacheBustedUrl);
      toast.success("Transformations applied! Image may take a few seconds to load.");
    } catch (error) {
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-slate-800/50 rounded-lg border border-slate-700">
                    ...
                    <p className="text-sm text-slate-400">
                      AI-powered background removal (ImageKit e-bgremove)
                    </p>
                  </div>
                      ...
                      {imageLoadError ? (
                        <div className="text-center py-12 space-y-4">
                          <p className="text-yellow-400">
                            ⚠️ Image transformation is processing...
                          </p>
                          <p className="text-sm text-slate-400">
                            AI transformations can take 5-10 seconds
                          </p>
                          <Button
                            onClick={() => {
                              setImageLoadError(false);
                              setTransformedImage(`${transformedImage.split('?')[0]}?t=${Date.now()}`);
                            }}
                            variant="outline"
                            size="sm"
                          >
                        ...
                        <img
                          ...
                          onError={() => {
                            ...
                            toast.error("Image still processing. Wait a moment and click Retry.");
                          }}
                        />
                      )}
                    </div>
